...
const outputPath = resolve(directory, "output/activation-preflight.json");
...
async function inspect(label, url, artifact) {
  const selectors = artifact.methodIdentifiers;
  const transaction = { from: OPERATOR, to: FACTORY, value: "0x0", data: CALLDATA };
  const [
    ...
    rpc(url, "eth_getCode", [FACTORY, "latest"]),
    rpc(url, "eth_call", [{ to: FACTORY, data: `0x${selectors["launchesPaused()"]}` }, "latest"]),
    rpc(url, "eth_call", [{ to: FACTORY, data: `0x${selectors["launchCount()"]}` }, "latest"]),
    rpc(url, "eth_call", [{ to: FACTORY, data: `0x${selectors["isLaunchConfigurationValid()"]}` }, "latest"]),
    rpc(url, "eth_call", [{ to: FACTORY, data: calldataForAddress(`0x${selectors["creatorAllowed(address)"]}`, OPERATOR) }, "latest"]),
    rpc(url, "eth_call", [{ to: FACTORY, data: `0x${selectors["operator()"]}` }, "latest"]),
  ...
  return {
    ...
    approvedCreatorAllowed: bool(creatorHex),
...
export function validateActivationReports(primary, fallback) {
  ...
  for (const report of [primary, fallback]) {
    ...
    require(report.factoryPaused === true, `${report.label} factory is not paused`);
    require(report.factoryLaunchCount === 0, `${report.label} launch count is not zero`);
    require(report.factoryConfigurationValid === true, `${report.label} factory configuration is invalid`);
    require(report.approvedCreatorAllowed === true, `${report.label} approved creator is not allowed`);
...
export async function main() {
  ...
  const artifact = JSON.parse(await readFile(
    resolve(projectRoot, "v2/out/DoomLaunchFactoryV2.sol/DoomLaunchFactoryV2.json"),
    "utf8",
  ));
  const [primary, fallback] = await Promise.all([
    inspect("primary", primaryUrl, artifact),
    inspect("fallback", fallbackUrl, artifact),
  ]);
  const errors = validateActivationReports(primary, fallback);
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(error => {
    console.error(`V2 activation preflight failed: ${error.message}`);
    process.exitCode = 1;
  });
}
