...
export const PUBLIC_FACTORY = "DoomPublicLaunchFactoryV2";
...
export const STEPS = Object.freeze([
  ...
  { order: 4, kind: "CREATE", contract: PUBLIC_FACTORY },
...
export function validateManifest(manifest) {
  ...
  if (manifest?.economics?.firstLaunchId !== 2 || manifest?.economics?.finalLaunchId !== 100) {
    errors.push("manifest must reserve Genesis ID 1 and limit public IDs to 2..100");
  }
  return errors;
}
...
export function validatePlan(plan) {
  ...
  const bindings = [
    ...
    [5, created.DoomLaunchDeployerV2, created[PUBLIC_FACTORY]],
    [6, created.V3GraduationManagerV2, created[PUBLIC_FACTORY]],
  ];
...
export async function buildPlan(startingNonce) {
  ...
  const manifest = await readJson(resolve(projectRoot, "config/public-v2-mainnet-deployment-manifest.json"));
  ...
  for (const step of STEPS) {
    ...
    const argument = step.order === 3 ? predicted.V3GraduationManagerV2 : predicted[PUBLIC_FACTORY];
  ...
  const plan = {
    schemaVersion: 1,
    status: "unsigned_public_v2_transaction_plan",
    ...
    transactions,
    warning: "Unsigned plan only. Re-plan if the pending nonce changes. Deployment leaves the public factory paused.",
  };
...
export async function main(argv = process.argv.slice(2)) {
  ...
  console.log(`Unsigned seven-transaction public V2 plan for starting nonce ${plan.startingNonce}.`);
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(error => {
    console.error(`Public V2 transaction plan failed: ${error.message}`);
    process.exitCode = 1;
  });
}
