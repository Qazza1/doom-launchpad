...
interface IFullScaleGraduationManagerHealthV3 {
...
/// @title DoomFullScaleLaunchFactoryV3
/// @notice Permissionless EOA launch factory for the permanent public launchpad generation.
/// @dev IDs begin at 101 so every prior Genesis and capped-beta ID remains globally unique.
contract DoomFullScaleLaunchFactoryV3 is ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 public constant LAUNCH_FEE = 0.001 ether;
    uint256 public constant FIRST_LAUNCH_ID = 101;
    bool public constant UNBOUNDED_LAUNCHES = true;
        ...
        returns (uint256 launchId, address tokenAddress, address curveAddress, address escrowAddress)
    {
        ...
        address initializedPool =
            IFullScaleGraduationManagerHealthV3(graduationManager).initializeLaunchPool(curveAddress);
    ...
    function isLaunchConfigurationValid() public view returns (bool) {
        ...
        try IFullScaleGraduationManagerHealthV3(graduationManager).isNetworkConfigurationValid() returns (bool valid) {
            return valid;
        } catch {
            return false;
        }
