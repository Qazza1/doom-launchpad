...
/// @notice Allowlisted 100-launch beta factory for curve-funded launches and automatic V3 graduation.
contract DoomLaunchFactoryV2 is ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 public constant LAUNCH_FEE = 0.001 ether;
    uint256 public constant MAX_LAUNCHES = 100;
    ...
    struct FactoryConfig {
        ...
        address initialApprovedCreator;
        address treasury;
        address doomRewards;
    ...
    mapping(address creator => bool allowed) public creatorAllowed;
    ...
    event CreatorPermissionChanged(address indexed creator, bool allowed);
    ...
    constructor(FactoryConfig memory config_) {
        if (
            config_.operator == address(0) || config_.emergencyGuardian == address(0)
                || config_.initialApprovedCreator == address(0) || config_.treasury == address(0)
        ...
        if (config_.initialApprovedCreator.code.length != 0) {
            revert ContractCreatorNotAllowed(config_.initialApprovedCreator);
        }
        ...
        creatorAllowed[config_.initialApprovedCreator] = true;
        emit CreatorPermissionChanged(config_.initialApprovedCreator, true);
        emit LaunchPauseChanged(true, address(this));
    }

    function setCreatorAllowed(address creator, bool allowed) external onlyOperator {
        if (creator == address(0)) revert ZeroAddress();
        if (creator.code.length != 0) revert ContractCreatorNotAllowed(creator);
        creatorAllowed[creator] = allowed;
        emit CreatorPermissionChanged(creator, allowed);
    }
        ...
        returns (uint256 launchId, address tokenAddress, address curveAddress, address escrowAddress)
    {
        ...
        if (!creatorAllowed[msg.sender]) revert UnauthorizedCreator(msg.sender);
        if (msg.sender != tx.origin || msg.sender.code.length != 0) revert ContractCreatorNotAllowed(msg.sender);
        if (nextLaunchId > MAX_LAUNCHES) revert LaunchLimitReached(MAX_LAUNCHES);
    ...
    function isLaunchConfigurationValid() public view returns (bool) {
        ...
        try IGraduationManagerHealthV2(graduationManager).isNetworkConfigurationValid() returns (bool valid) {
            return valid;
        } catch {
            return false;
        }
